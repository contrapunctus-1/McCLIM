...
;;; CLIM's input kernel is only hinted in the specification. This class make
...
;;; This function is like "peek-no-hang" but only locally in the buffer, i.e it
;;; does not trigger wait on event-queue.
        ...
        (multiple-value-bind (available reason)
          ...
          (if available
              ...
              (case reason
                ...
                #+ (or)
                ;; EOF may be returned by input streams backed by a string
                ;; (see the "XXX Evil hack" below), but that should not happen
...
(defmethod stream-process-gesture
  ...
  (declare (ignore type))
  (if-let ((char (and (typep gesture 'key-press-event)
                      (event-char gesture))))
